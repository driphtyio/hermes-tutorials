...
- **State Status:** `PENDING` → `EXECUTING` → `AWAITING_REVIEW` → `COMPLETED`
...
- **[Custom Tool Integration](./custom-tools)** — Creating specialized tools for your subagents
- **[Memory Management](./memory-management)** — Managing shared state and global variables across parallel branches
- **[API Reference](./api-docs)** — Technical documentation for `delegate_task` and `worktree` endpoints
