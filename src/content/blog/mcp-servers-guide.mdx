...
description: "MCP connects Hermes Agent to external tools via CLI (`hermes mcp add`) or `config.yaml` (env, headers). Stdio and HTTP servers. Per-server filtering: whitelist/blacklist with `mcp_server_filter`; disable servers. Built-in presets: GitHub, filesystem, Playwright, Stripe, Brave Search, Memory. Utility wrappers for resources/prompts; filter with `filter_utilities`. Hermes serves as MCP server (`hermes mcp serve`) exposing 10 messaging tools. Test via `hermes mcp test`; changes on `/reload-mcp`; ..."
pubDate: "2026-05-25"
