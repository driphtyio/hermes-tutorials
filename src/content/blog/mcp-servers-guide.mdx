...
description: "MCP uses stdio or HTTP. Add servers via CLI (--command/--args e.g., filesystem npx) or YAML. Presets built-in. Filtering: whitelist, blacklist, filter_utilities, disable server. Hermes serves MCP server exposing 10 tools: conversations_list, messages_read, messages_send, channels_list, events_poll, events_wait, attachments_fetch, conversation_get, permissions_list_open, permissions_respond. Client config JSON (Claude Code) uses venv path. Test with hermes mcp test. Resource/prompt utilities c..."
pubDate: "2026-05-25"
