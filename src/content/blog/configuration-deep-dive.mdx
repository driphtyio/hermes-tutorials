...
Hermes loads environment variables from `~/.hermes/.env`. This file keeps secrets separate from `config.yaml` and is excluded from config checksums.
...
| `AI_GATEWAY_API_KEY` | AI Gateway (Vercel) |
| `OPENCODE_ZEN_API_KEY` | OpenCode Zen |
| `OPENCODE_GO_API_KEY` | OpenCode Go |
| `COPILOT_GITHUB_TOKEN` | GitHub Copilot (token auth) |

### Voice & Media

| Variable | Purpose |
|----------|---------|
| `VOICE_TOOLS_OPENAI_KEY` | OpenAI TTS and STT |
| `ELEVENLABS_API_KEY` | ElevenLabs TTS |
| `MISTRAL_API_KEY` | Mistral STT (Voxtral) |
| `REPLICATE_API_TOKEN` | Replicate image generation |

### OAuth Providers

For OAuth-based providers, use `hermes login` instead of env vars:

```bash
hermes login --provider nous          # Nous Portal
hermes login --provider openai-codex  # OpenAI Codex
```

The `hermes model` interactive picker guides you through OAuth flows automatically when you select an OAuth provider.

### Custom Endpoints

For custom OpenAI-compatible endpoints, use `config.yaml`:
