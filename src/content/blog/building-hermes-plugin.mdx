...
description: "Hermes plugins add custom tools, hooks, slash commands, and CLI subcommands without modifying core code. A time-tracker plugin built in ~150 Python lines demonstrates this: it includes a timezone_convert tool, pomodoro timer, /focus slash command, and post_tool_call hook for logging. The plugin lives in `~/.hermes/plugins/time-tracker/` with four files: manifest `plugin.yaml`, `__init__.py` (with `register(ctx)` entry point), `schemas.py`, and `tools.py`. Tool handlers return JSON strings, al..."
pubDate: "2026-05-22"
